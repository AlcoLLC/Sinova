...
def submit_investor_cat_to_google(sender, instance, created, **kwargs):
    ...
    for lang_code in language_codes:
        try:
            activate(lang_code)
            path = reverse('investorRelation:category_detail', kwargs={'category_slug': instance.slug})
...
def delete_investor_cat_from_google(sender, instance, **kwargs):
    language_codes = [lang[0] for lang in settings.LANGUAGES]
    original_lang = get_language()
